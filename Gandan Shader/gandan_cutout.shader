...
Shader "gandan shader/Gandan Cutout"
{
    Properties
    {
        ...
        [PowerSlider(2.0)]_UnlitThreshold("Lit <-> Unlit", Range(0, 1)) = 0
        [Space(20)]
        [Enum(UnityEngine.Rendering.CullMode)] _Cull("Culling Mode (Off = Double Sided)", Float) = 2
    }
    SubShader
    {
        Tags {
        ...
        Pass
        {
            ...
            float4 frag (vertexOutput i) : SV_Target
            {
                ...
                float4 finalColor = float4(atten * color * NdotL, 1);
                clip(tex.a - 0.5);
